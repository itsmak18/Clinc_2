...
- `assertPatientInScope(req, patientId, entityType?)` — resolves silently when allowed; **throws `ForbiddenError`** on deny (logs `logAudit("ACCESS_DENIED_OUT_OF_SCOPE", …)` first). The throw routes through `asyncHandler` to the canonical error envelope. Do not wrap the call in `if (!await …)` — the bool-returning shape was removed 2026-05-30 because forgetting the check silently granted access.
- `assertMedicalRecordInScope(req, recordId)` — Rule 1: `doctorId=current user` → allow (resolves silently); Rule 2: deny → `logDenied("record_not_owned")` + throws `ForbiddenError`. Record-not-found resolves silently so the caller handles 404.
...
- `GET /medical-records/:id` — `assertMedicalRecordInScope` (doctorId ownership only)
...
- **Never put `SESSION_SECRET`, `FIELD_ENCRYPTION_KEY`, or `METRICS_TOKEN` in the `environment:` block of `docker-compose.prod.yml`.** These are file-mounted via Docker secrets since 2026-05-27. Adding them back to `environment:` exposes them to `docker inspect`. **Exception (2026-05-31 Phase 4)**: Alertmanager SMTP credentials (`SMTP_AUTH_PASS` et al.) ARE in `environment:` because Alertmanager's YAML supports `${VAR}` interpolation but not file-based secrets without a sidecar renderer. Use an app-specific SMTP token, never a reusable password.
