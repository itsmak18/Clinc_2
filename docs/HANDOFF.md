...
- **Admin sidebar visibility** — `super_admin/admin` see a _grouped_ menu driven by the hardcoded `ADMIN_SECTIONS` array in `components/Layout.tsx`; new pages (`reconciliation`, `service-prices`) were invisible until their keys were added there. **Recurring footgun — see Gotchas.**
