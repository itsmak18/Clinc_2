...
> As of 2026-05-28: backend remediation phases 0, 1, 3, 4, 5 complete; Phase 2 (Auth) code-landed flag-OFF. **Bayan Design Port complete:** Phases 0-5 shipped as of 2026-05-27. **Security hardening bundle (2026-05-27):** trust proxy, secrets file-mount, cursor pagination fully wired, route-access contract test, CI security tools (CodeQL+Trivy+SBOM), field-encryption KID envelope — all complete. **M3 — EdDSA + JWKS (2026-05-28):** JWT migrated from HS256 to asymmetric Ed25519. **M8 — Drop `invoices.items` JSONB (2026-05-28):** invoice_items normalized table is sole source of truth. **P1-1 — Audit transactional outbox (2026-05-28):** audit trail durable. **P1-9 — Erasure ↔ backup blackout (2026-05-28).** **P1-8 — SSE graceful drain (2026-05-28).** **P0-4 — clinic_id service-layer enforcement (2026-05-28):** JWT carries clinicId, all 10 PHI service files filter by `clinic_id = user.clinicId`, migration `0008_acoustic_cassandra_nova.sql`. **379/379 tests passing.**
...
- ⏳ **OpenAPI sync + codegen** — new routes (`/auth/verify-device`, `/auth/wasnt-me`, `/account/devices`, `/auth/forgot-password`, `/auth/reset-password`, `/auth/admin-reset/:userId`, `/api/csp-report`) + `pending_verification` response + `dvu` JWT claim.
- ⏳ **Drizzle migration generation** — run `pnpm --filter @workspace/db db:generate` then `db:migrate` (staging then prod).
- ⏳ **Test coverage for Phase 2** — token atomicity, fingerprint binding, role-branched flow, byte-identical enumeration response, HIBP rejection.
