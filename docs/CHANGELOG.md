...
- **Frontend:** required ID Card field at the top of the Register-Patient dialog; Save disabled until it (and the other required fields) are filled; new ID Card column in the patient table + CSV export. EN/AR i18n key `idCardNumber` ("ID Card Number" / "رقم الهوية").
...
- **Latent prod risk surfaced (NOT yet fixed):** `mrn_seq` is created only by the **dev seed** (`CREATE SEQUENCE IF NOT EXISTS mrn_seq`), never by a migration. The seed truncates all data, so it is not run in prod — meaning a freshly-migrated prod DB has **no `mrn_seq`**, and the first real registration would fail with `relation "mrn_seq" does not exist`. Has never bitten (app undeployed). **Recommended:** move `mrn_seq` (and `invoice_seq`) creation into a migration. The new integration test creates `mrn_seq` in `beforeAll` to mirror the seed.
- **Diagnostic gap noted:** the registration `onError` toast (`patientRegisterFailed`) is generic and hid both the 400 and the 500. Surfacing `error_code`/`message` from the canonical envelope remains an open follow-up.
