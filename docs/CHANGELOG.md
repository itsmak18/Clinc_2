...
- **i18n coverage pass.** Routed every remaining user-facing string through `t()` and added **97 keys** (EN + AR; dictionary now 995/995, parity verified). Screens fixed: Triage (kanban columns, quick-vitals, full-vitals dialog, priority labels), [ForgotPassword.tsx](../artifacts/clinic/src/pages/ForgotPassword.tsx) and [AccountDevices.tsx](../artifacts/clinic/src/pages/AccountDevices.tsx) (neither imported `useI18n` at all), [TweaksPanel.tsx](../artifacts/clinic/src/components/TweaksPanel.tsx), [DayScheduleView.tsx](../artifacts/clinic/src/components/DayScheduleView.tsx), [LiveActivityFeed.tsx](../artifacts/clinic/src/components/LiveActivityFeed.tsx), [PatientTimeline.tsx](../artifacts/clinic/src/components/PatientTimeline.tsx), [GlobalSearch.tsx](../artifacts/clinic/src/components/GlobalSearch.tsx), the booking-source badges in [Appointments.tsx](../artifacts/clinic/src/pages/Appointments.tsx), Layout/Login aria-labels, and the example-placeholders on Lab/XRay/Ultrasound/Inventory/ServicePrices. Intentional keeps: the **Wateen** brand mark, keyboard keys (`Esc`/`⌘K`/`↵`), units/codes (`g/dL`, `OR-1`, `CONSULTATION`), and the palette/voice/density design-token names.
- **Choose-your-print-language.** [print.ts](../artifacts/clinic/src/lib/print.ts) — all 6 templates (prescription, blank prescription, lab, x-ray, ultrasound, invoice) gained an optional `lang: PrintLang = "en"` param + a self-contained bilingual label dictionary + RTL direction handling. A new global [printLang.tsx](../artifacts/clinic/src/hooks/printLang.tsx) provider (`usePrintLang().choosePrintLanguage()`) pops an **English / العربية** dialog before each print; wired into Prescriptions (×4), Lab, XRay, Ultrasound, Billing (×2). [DischargeSheet.tsx](../artifacts/clinic/src/components/DischargeSheet.tsx) got an inline EN/AR segmented toggle (WYSIWYG — preview + print follow the choice, labels resolved via `translations[printLang]`, body `dir` flips). PHI free-text still prints as entered; only fixed labels + direction switch. (Not yet localized: the admin **Reports** analytics print.)
- **Login language toggle.** [Login.tsx](../artifacts/clinic/src/pages/Login.tsx) now has a globe **language button** (top-end of the form panel) so staff can pick Arabic/English *before* signing in.

**Verify:** clinic `tsc -p tsconfig.json --noEmit` exit 0; clinic `vitest run` **51/51** (updated `discharge-sheet-xss.test.tsx` to wrap the render in `I18nProvider`, since DischargeSheet now uses `useI18n`).

## Doctors can edit a patient's allergies (2026-06-23)

Doctors can now record/update the clinical **allergies** field on a patient — previously only `super_admin`/`admin` could (front_desk's edit form omits allergies; nurses have no edit path). Scoped to the allergies column only; demographics/contact PII stay off-limits to doctors.

- **Backend (narrow, scope-enforced).** `PATCH /patients/:patientId` now also allows `doctor` ([patients.ts](../artifacts/api-server/src/routes/patients.ts)). `updatePatient` ([patients.service.ts](../artifacts/api-server/src/services/patients.service.ts)) gained a `role === "doctor"` branch that **whitelists `{ allergies }` only** (so extra body fields are ignored even if sent), and now calls `assertPatientInScope` for doctor-scoped roles **before** mutating — a doctor can only edit allergies for their assigned patients (or under an active break-glass session); out-of-scope → `ForbiddenError`. Allergies are AES-256-GCM encrypted at rest (existing `encryptNullable` path) and the change is audited (`UPDATE` patient; `auditSnapshot` redacts the encrypted field, `details.fields = ["allergies"]`). No OpenAPI change — `UpdatePatientBody.allergies` already exists and is optional, so the existing `useUpdatePatient` hook is reused.
- **Frontend.** [PatientDetail.tsx](../artifacts/clinic/src/pages/PatientDetail.tsx): doctors get an **Edit allergies** button in the patient toolbar (they don't have the full Edit dialog) opening a focused single-field dialog, prefilled from the current value, saving via `useUpdatePatient({ allergies })`. The allergies banner + info card refresh on the summary-query invalidation.
- **i18n.** Added `editAllergies`, `allergiesUpdated`, `allergiesPlaceholder` (EN + AR).

**Verify:** clinic + api-server `tsc -p tsconfig.json --noEmit` exit 0; api-server route `eslint src/routes` exit 0.

## Patient Detail — hide the Timeline tab from doctors (2026-06-23)

Doctors no longer see the **Timeline** tab on the patient detail page ([PatientDetail.tsx](../artifacts/clinic/src/pages/PatientDetail.tsx)) — both the `TabsTrigger` and its `TabsContent` are now gated `role !== "doctor"`, leaving doctors with the Overview tab only. Rationale matches the existing pattern on that page (doctors already have the "Recent Appointments" card hidden): a doctor works from the clinical record, not the patient's cross-clinic visit history. Frontend-only, read-only; the doctor's Consult page keeps its own clinical **History** tab (unchanged). clinic `tsc -p tsconfig.json --noEmit` exit 0.

## Lab results read-only for doctors + restore Request button on imaging (2026-06-23)

The **Lab** page ([Lab.tsx](../artifacts/clinic/src/pages/Lab.tsx)) let a doctor edit result parameters and pick any status — including marking a test **completed/cancelled** — even though the backend `PATCH /lab/tests/:testId` is already restricted to `super_admin, admin, lab_staff` ([lab.ts:37-38](../artifacts/api-server/src/routes/lab.ts#L37-L38)). So a doctor's Save silently 403'd (the lab `updateMutation` has no `onError` handler). X-Ray/Ultrasound already gate their result editor behind a `canEdit` role flag; Lab did not. Aligned Lab to the same pattern and, per request, made the **Request** button visible to doctors on all three diagnostics pages.

- **Lab editor now staff-only (read-only for doctors).** Added `EDIT_ROLES = ["super_admin", "admin", "lab_staff"]` + `canEdit` to Lab.tsx (mirrors [XRay.tsx:26](../artifacts/clinic/src/pages/XRay.tsx#L26) / [Ultrasound.tsx:30](../artifacts/clinic/src/pages/Ultrasound.tsx#L30)). When `!canEdit`: parameter inputs + flag selects + notes are `disabled`, the Add-row / delete-row controls and the **Status** select are hidden, the **Save** button is hidden, and the footer **Cancel** becomes **Close**. Doctors keep the read-only parameter grid + **Print**. No change to the lab tech / admin experience (they remain in `EDIT_ROLES`). The Lab page's only non-edit role is `doctor` ([route-access.ts:124-129](../artifacts/clinic/src/lib/route-access.ts#L124-L129)).
- **Request button restored for doctors on imaging.** X-Ray/Ultrasound previously hid the "New Request" button behind `canEdit` (doctors had to deep-link from the patient page). Un-gated it on both pages — every role that can open the page can also order a study (create-roles ⊇ page-access roles for all three modalities), so no new 403 surface. Results editing stays gated by `canEdit`.
- **No backend / API / route-access / i18n changes.** Pure frontend RBAC-UX gating; reuses existing translation keys (`close`, `cancel`, `save`). Doctor-scope, audit, and the diagnostics state machine are unaffected.

**Verify:** clinic `tsc -p tsconfig.json --noEmit` exit 0; clinic `vitest run` 50/51 (the 1 failure is the pre-existing EN/AR key-parity check, unrelated — no strings were added).

## Clinical lists — click a row to open/expand the full record (2026-06-23)

The doctor's **My Orders** list ([DoctorOrders.tsx](../artifacts/clinic/src/pages/DoctorOrders.tsx)) was display-only — the **Result** column truncated to one line (and, worse, showed the raw JSON envelope, e.g. `{"v":1,"findings":"","impression":""}`, when a study had no report yet). Wired the existing `DataTable` `onRowClick`/`expandedRow` props so a doctor can click any order to read it in place.

- **Click-to-expand detail panel.** Clicking a row toggles an inline read-only panel: a patient/order meta strip (modality, patient, MRN, date, status) plus the full result — lab orders render the parameter grid (param/value/unit/ref-range/flag with H/L highlighting), X-ray/ultrasound render **Findings** + **Impression**. A chevron column signals expandability; the expanded row is highlighted. Expand key is composite (`${kind}:${id}`) since lab/xray/ultrasound ids can collide.
- **Cleaner collapsed preview.** The Result column now derives a human-readable one-line summary from the parsed payload instead of dumping raw JSON; empty/unreported results show `—`, and the expanded panel shows "Result pending — not yet reported".
- **Jump to chart.** A **View patient** button in the panel navigates to `/patients/:id` (wouter `setLocation`, same pattern as `Patients.tsx`).
- **Consult → Summary "Recent Medical Records" now expandable.** Same dead-click bug in [DoctorConsult.tsx](../artifacts/clinic/src/pages/DoctorConsult.tsx): each record was a non-interactive `<div>` (diagnosis + chief complaint + date only). Made each row a button that expands inline to reveal **Doctor**, **Treatment**, and **Notes** (falls back to "No additional details recorded"); chevron rotates, open record resets on patient switch.
- **Patient Detail → Overview "Recent Medical Records" now opens the record.** Same dead-click in [PatientDetail.tsx](../artifacts/clinic/src/pages/PatientDetail.tsx): the X-Ray/Lab rows already opened `DiagnosticResultDialog`, but medical-record rows were inert `<div>`s. Made each row clickable (keyboard-accessible) to open the full record. Extracted the record detail popup into a shared [MedicalRecordDetailDialog.tsx](../artifacts/clinic/src/components/MedicalRecordDetailDialog.tsx) (patient/doctor/date, chief complaint, diagnosis, treatment, notes, vitals with abnormal flagging) and **refactored `MedicalRecords.tsx` to consume it** — dropping its duplicated inline dialog + `renderVitals`/`VITALS_VIEW` so the layout lives in one place (mirrors `DiagnosticResultDialog`). `isAbnormal`/`VITAL_NORMAL` stay in `MedicalRecords.tsx` for the create-form live flagging.
- **i18n.** Added `viewPatient`, `resultPending`, `noAdditionalDetails` (EN + AR). All other labels reuse existing keys. No backend/API/route changes; all read-only — doctor-scope and audit are unaffected.

**Verify:** clinic `tsc -p tsconfig.json --noEmit` exit 0.
