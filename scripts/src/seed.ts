...
async function seed() {
  ...
  const [superAdmin, admin, doctor1, doctor2, nurse, frontDesk, xrayStaff, labStaff] = await db.insert(usersTable).values([
    { username: "superadmin", passwordHash: await hashPassword("admin123"), fullName: "Super Administrator", fullNameAr: "مشرف عام", email: "superadmin@medicore.com", role: "super_admin" },
    { username: "admin", passwordHash: await hashPassword("admin123"), fullName: "Admin User", fullNameAr: "مشرف", email: "admin@medicore.com", role: "admin" },
    { username: "dr_ahmed", passwordHash: await hashPassword("doctor123"), fullName: "Dr. Ahmed Al-Rashidi", fullNameAr: "د. أحمد الراشدي", email: "ahmed@medicore.com", role: "doctor", phone: "+966501234567" },
    { username: "dr_sara", passwordHash: await hashPassword("doctor123"), fullName: "Dr. Sara Al-Nasser", fullNameAr: "د. سارة الناصر", email: "sara@medicore.com", role: "doctor", phone: "+966502234567" },
    { username: "nurse1", passwordHash: await hashPassword("nurse123"), fullName: "Fatima Al-Zahra", fullNameAr: "فاطمة الزهراء", email: "fatima@medicore.com", role: "nurse" },
    { username: "receptionist", passwordHash: await hashPassword("front123"), fullName: "Omar Al-Farouk", fullNameAr: "عمر الفاروق", email: "omar@medicore.com", role: "front_desk" },
    { username: "xray_tech", passwordHash: await hashPassword("xray123"), fullName: "Khalid Al-Mutairi", fullNameAr: "خالد المطيري", email: "khalid@medicore.com", role: "xray_staff" },
