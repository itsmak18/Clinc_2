...
router.get("/inventory/:itemId", async (req, res) => {
  const [item] = await db.select().from(inventoryTable).where(eq(inventoryTable.id, parseInt(req.params.itemId)));
...
router.patch("/inventory/:itemId", async (req: AuthRequest, res) => {
  ...
  const [item] = await db.update(inventoryTable)
    ...
    .where(eq(inventoryTable.id, parseInt(req.params.itemId)))
    .returning();
