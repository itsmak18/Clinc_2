...
router.get("/dashboard/summary", async (_req, res) => {
  ...
  const [
    ...
    db.select({ total: invoicesTable.total, status: invoicesTable.status }).from(invoicesTable).where(and(isNull(invoicesTable.deletedAt), gte(invoicesTable.createdAt, yesterdayStart), lte(invoicesTable.createdAt, yesterdayEnd))),
  ]);
...
router.get("/dashboard/department-load", requireRole("super_admin", "admin"), async (_req, res) => {
  const now = new Date();
  const todayStart = new Date(now.getFullYear(), now.getMonth(), now.getDate());
  const todayEnd = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 23, 59, 59);
