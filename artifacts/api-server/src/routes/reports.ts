...
router.get("/reports/appointments", async (req, res) => {
  ...
  const averageWaitTimeMinutes = waitTimes.length > 0 ? waitTimes.reduce((s, t) => s + t, 0) / waitTimes.length : 0;
  ...
  res.json({ totalAppointments: total, completed, cancelled, cancellationRate, averageWaitTimeMinutes, byDoctor });
});
...
router.get("/reports/revenue", async (req, res) => {
  ...
  res.json({ totalRevenue, totalInvoices, paidInvoices, averageInvoiceValue, byDay });
});
